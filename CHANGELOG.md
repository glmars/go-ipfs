...
maintain between 600 and 900 open connections. This is still likely higher than
needed, and future releases may lower the default recommendation, but for now
we want to make changes gradually. The rationale for this selection of numbers
is as follows:
...
noticed that the add progressed about 8MB at a time, pausing shortly in between.
This was caused by quickly filling up the batch, then blocking while it was
written to disk. We now write to disk in the background while continuing to add
the remainder of the file.

Other changes in this release have noticably reduced memory consumption and cpu
